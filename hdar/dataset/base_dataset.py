...
class BaseDataset(Dataset):
    ...
    def __init__(self, options, dataset, ignore_3d=False,
        ...
        if dataset == 'h36m':
            str_array = self.data['imgname']
            # 用于存储修改后的字符串
            modified_array = np.empty(len(str_array), dtype=object)
            ...
            for i, s in enumerate(str_array):
                ...
                if first_slash_index != -1 and second_slash_index != -1:
                    modified_str = s[:first_slash_index] + s[second_slash_index:]
                else:
                    modified_str = s
                modified_array[i] = modified_str

            self.imgname = modified_array
            # #self.imgname = self.data['imgname']
    ...
    def __getitem__(self, index):
        ...
        try:
            ...
            print("这里跑起来了么")
            print(f"Failed to load image: {self.img_dir}")
            logger.info(imgname)
