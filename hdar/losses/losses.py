...
class PARELoss(nn.Module):
    ...
    def forward(self, pred, gt):
        ...
        loss_dict = {
            ...
            'loss/loss_dimension': loss_dimension,
        }
...
def compute_length(vertices, index_list, axis=1):
    """计算两点之间在某个轴上的绝对距离"""
    a = vertices[:, index_list[0], axis]  # 第一个点的 X/Y/Z 坐标
    b = vertices[:, index_list[1], axis]  # 第二个点的 X/Y/Z 坐标
    return torch.abs(a - b)  # 计算距离


def compute_circumference(vertices, index_list):
    """计算周长（多个点围成的封闭区域的总长）"""
    circumference = torch.zeros(vertices.shape[0], device=vertices.device)
    for i in range(len(index_list) - 1):
        a = vertices[:, index_list[i], :]
        b = vertices[:, index_list[i + 1], :]
        circumference += torch.norm(a - b, dim=1)  # 计算欧式距离
    return circumference


def dimension_loss(pred_vertices, gt_vertices, has_smpl, criterion):
    """计算所有人体测量维度的损失"""
    ...
    # 周长测量
    circumference_dimensions = {
        "颈围": [3057, 451, 214, 217, 151, 152, 297, 207, 208, 210, 211, 256, 424, 3164, 3919, 3769, 3725, 3722, 3720,
               3721, 3809, 3664, 3665, 3727, 3726, 3942, 3057],
        "胸围": [6489, 304, 3040, 3041, 3044, 3043, 612, 1197, 1763, 1422, 644, 643, 737, 763, 2906, 1253, 1236, 750, 751,
               3014, 4239, 4240, 4719, 4736, 6366, 4252, 4251, 4224, 4133, 4132, 4896, 5230, 4098, 4100, 6490, 6491,
               6487, 6488, 6489],
    }
    ...
    if len(gt_vertices_with_shape) > 0:
        total_loss = torch.tensor(0.0, device=pred_vertices.device)

        # 长度损失 (Y 轴计算)
        for key, indices in length_dimensions.items():
            pred_value = compute_length(pred_vertices_with_shape, indices, axis=1)
            gt_value = compute_length(gt_vertices_with_shape, indices, axis=1)
            total_loss += criterion(pred_value, gt_value)

        # 宽度损失 (X 轴计算)
        for key, indices in width_dimensions.items():
            pred_value = compute_length(pred_vertices_with_shape, indices, axis=0)
            gt_value = compute_length(gt_vertices_with_shape, indices, axis=0)
            total_loss += criterion(pred_value, gt_value)

        # 深度损失 (Z 轴计算)
        for key, indices in depth_dimensions.items():
            pred_value = compute_length(pred_vertices_with_shape, indices, axis=2)
            gt_value = compute_length(gt_vertices_with_shape, indices, axis=2)
            total_loss += criterion(pred_value, gt_value)

        # 周长损失
        for key, indices in circumference_dimensions.items():
            pred_value = compute_circumference(pred_vertices_with_shape, indices)
            gt_value = compute_circumference(gt_vertices_with_shape, indices)
            total_loss += criterion(pred_value, gt_value)
