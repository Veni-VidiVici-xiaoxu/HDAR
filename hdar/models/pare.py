...
class PARE(nn.Module):
    def __init__(#初始化好多都给了none
        ...
        super(PARE, self).__init__()#调用了 nn.Module 的构造函数
        if backbone.startswith('hrnet'):#参数文件--hrnet_w32-conv
            backbone, use_conv = backbone.split('-')#将backbone参数分成 hrnet的宽度和是否使用卷积
            ...
            self.backbone = eval(backbone)(pretrained=True)#eval函数动态执行字符串，创建backbone实例

        # self.backbone = eval(backbone)(pretrained=True)
        self.head = PareHead(
            num_joints=num_joints,
            num_input_features=get_backbone_info(backbone)['n_output_channels'],#输入特征数量
            softmax_temp=softmax_temp,#温度参数是个啥子东西
            num_deconv_layers=num_deconv_layers,#反卷积层的数量，过滤器数量，卷积核大小
            num_deconv_filters=[num_deconv_filters] * num_deconv_layers,
            num_deconv_kernels=[deconv_conv_kernel_size] * num_deconv_layers,
            num_features_smpl=num_features_smpl,#smpl特征数量
            final_conv_kernel=1,#最终卷积核大小
            iterative_regression=iterative_regression,#是否使用迭代回归
            iter_residual=iter_residual,#迭代的残差
            num_iterations=num_iterations,#迭代的次数
            shape_input_type=shape_input_type,#输入类型
            pose_input_type=pose_input_type,
            pose_mlp_num_layers=pose_mlp_num_layers,#mlp层配置
            ...
            use_keypoint_features_for_smpl_regression=use_keypoint_features_for_smpl_regression,#关键点特征和注意力机制
            ...
            use_branch_nonlocal=use_branch_nonlocal, # 'concatenation', 'dot_product', 'embedded_gaussian', 'gaussian'#时候使用费局部和共注意力机制
            ...
            use_upsampling=use_upsampling,#上采样和软注意力机制
            use_soft_attention=use_soft_attention,
            num_branch_iteration=num_branch_iteration,#分支迭代的次数和深度
            branch_deeper=branch_deeper,#控制了是否在resnet和hrnet之间切换卷积风格
            use_resnet_conv_hrnet=use_resnet_conv_hrnet,
            use_position_encodings=use_position_encodings,#是否使用位置编码和平均的相机形状/姿态。
            use_mean_camshape=use_mean_camshape,
            use_mean_pose=use_mean_pose,
            init_xavier=init_xavier,#权重初始化
        )

        self.use_cam = use_cam#将构造函数传入的 use_cam 参数存储为类的实例变量，用于后续判断是否使用相机参数。
        if self.use_cam:#为true时实例化smplcamhead类
    ...
    def forward(
        ...
        hmr_output = self.head(features, gt_segm=gt_segm)#生成hmr输出

        if self.use_cam:#默认=false，这个参数到底是用来干嘛的呀，不是很明白
            ...
            if isinstance(hmr_output['pred_pose'], list):#如果输出了多组smpl参数，为啥会有多组，不明白
    ...
    def load_pretrained(self, file):
        ...
        state_dict = torch.load(file)#加载预训练的权重，通常是一个.pt或.pth文件，包含了模型的参数
        self.backbone.load_state_dict(state_dict, strict=False)#将加载的状态字典应用到模型的backbone
