...
#smpl类负责处理与smpl模型相关的关节和顶点计算，SMPL 是一个3D人体模型，它定义了人体的几何结构和外观，而 SMPLHead 是一个神经网络组件，它利用 SMPL 模型的参数来预测人体的3D姿态和形状。
class SMPL(_SMPL):
    """ Extension of the official SMPL implementation to support more joints """

    def __init__(self, *args, **kwargs):
        super(SMPL, self).__init__(*args, **kwargs)
        joints = [constants.JOINT_MAP[i] for i in constants.JOINT_NAMES]#从常量中获取关节映射，
        J_regressor_extra = np.load(config.JOINT_REGRESSOR_TRAIN_EXTRA)#加载额外的关节回归器
        self.register_buffer('J_regressor_extra', torch.tensor(J_regressor_extra, dtype=torch.float32))
        self.joint_map = torch.tensor(joints, dtype=torch.long)

    def forward(self, *args, **kwargs):
        kwargs['get_skin'] = True
        smpl_output = super(SMPL, self).forward(*args, **kwargs)#调用父类的forward方法以获取smpl输出
        extra_joints = vertices2joints(self.J_regressor_extra, smpl_output.vertices)#计算额外的关节位置，并将其与原始关节位置合并
        ...
        return output#返回一个包括顶点、关节、姿态等信息的输出字典

#smplhead类负责将输入的姿态和形状转为3d模型的输出
class SMPLHead(nn.Module):
    ...
    def forward(self, rotmat, shape, cam=None, normalize_joints2d=False):
        ...
        output = {#区别好像就在于，smplhead输出的只是顶点和3d关节点
