...
class PareHead(nn.Module):
    def __init__(
            ...
            use_soft_attention=False, # Stefan & Otmar 3DV style attention 实际配置也是false
        ...
        if use_soft_attention:#使用软注意力机制，
        ...
        if use_hmr_regression:#HMR回归配置，
            self.iterative_regression = False#将 iterative_regression 设置为 False，以确保不使用迭代回归方法。
            logger.warning('iterative_regression cannot be used together with hmr regression')

        if self.use_heatmaps in ['part_segm', 'attention']:#表示需要使用部分分割或注意力机制
        ...
        if use_position_encodings:
            ...
            self.register_buffer('pos_enc', get_coord_maps(size=56))#get_coord_maps 函数生成坐标映射，这些映射将作为额外的特征与输入特征一起用于模型的后续处理。size=56 表示生成的坐标映射的大小。
            num_input_features += 2#更新输入特征的数量
            self.num_input_features = num_input_features
        ...
        if backbone.startswith('hrnet'):
            ...
            self.keypoint_deconv_layers = conv_fn(#构建关键点去卷积层
                num_deconv_layers,
                num_deconv_filters,
                num_deconv_kernels,
            )
            # reset inplanes to 2048 -> final resnet layer
            self.num_input_features = num_input_features#重置输入特征数量
        ...
        if self.use_soft_attention:#默认为false
            ...
            )#使用热图为part_segm或pool时输出通道数为num_joints+1
            ...
            self.keypoint_final_layer = nn.Conv2d(
                ...
                #out_channels=num_joints+1 if self.use_heatmaps in ('part_segm', 'part_segm_pool') else num_joints,#wmh要改的code
                out_channels=39,
        ...
        if self.use_hmr_regression:#默认是false
        ...
        if self.use_branch_nonlocal:#默认是null
    ...
    def _get_pose_mlp(self, num_joints, output_size):
        if self.pose_mlp_num_layers == 1:#确实为1
            return LocallyConnected2d(#这个层其实没看明白，不过应该无所谓吧，最后直接调用
                in_channels=self.pose_mlp_inp_dim,
    ...
    def forward(self, features, gt_segm=None):
        ...
        if self.use_position_encodings:#如果使用位置编码，将位置编码self.pos_enc重复features.shape[0]次（即批量大小），并与原始特征features沿着第一个维度（维度索引为1）进行拼接。这样可以为每个样本添加位置信息。
            features = torch.cat((features, self.pos_enc.repeat(features.shape[0], 1, 1, 1)), 1)

        output = {}

        ############## 2D PART BRANCH FEATURES ###############图1上半部分
        part_feats = self._get_2d_branch_feats(features)

        ############## GET PART ATTENTION MAP ##############
        part_attention = self._get_part_attention_map(part_feats, output)#这里输出的是heatmap

        ############## 3D SMPL BRANCH FEATURES ##############图的下半部分
        smpl_feats = self._get_3d_smpl_feats(features, part_feats)#为什么3d还是用2d作为输入了

        ############## SAMPLE LOCAL FEATURES ##############
        if gt_segm is not None:#真实的分割标签,默认=None
            # logger.debug(gt_segm.shape)
            # import IPython; IPython.embed(); exit()
            gt_segm = F.interpolate(gt_segm.unsqueeze(1).float(), scale_factor=(1/4, 1/4), mode='nearest').long().squeeze(1)#用最近邻插值方法将分割编码上采样4倍。
            part_attention = F.one_hot(gt_segm.to('cpu'), num_classes=self.num_joints + 1).permute(0,3,1,2).float()[:,1:,:,:]#将上采样的分割编码转换为one-hot编码格式
        ...
        if self.use_coattention:#默认=false
        ...
        if self.num_branch_iteration > 0:#默认=0
    ...
    def _get_local_feats(self, smpl_feats, part_attention, output):#用于提取局部特征和相机特征
        cam_shape_feats = self.smpl_final_layer(smpl_feats)
        #这里就这样简单的获得了shape和camera的形状？有点太草率了吧，我要在这里加一个人体维度的注意力机制，加个迭代之类的

        if self.use_keypoint_attention:#default=True
            point_local_feat = self.keypoint_attention(smpl_feats, part_attention)#使用keypoint_attention模块处理smpl分支特征，生成局部特征
            cam_shape_feats = self.keypoint_attention(cam_shape_feats, part_attention)
        else:#如果不使用关键点注意力，则使用插值方法interpolate，用于对特征图进行插值，以获取特定 UV 坐标处的特征。
    ...
    def _get_2d_branch_feats(self, features):
        part_feats = self.keypoint_deconv_layers(features)#这些特征图通常具有更高的空间分辨率，适合于关键点定位任务。
        if self.use_branch_nonlocal:#default为null 非局部块有助于捕获特征图中的长距离依赖关系，这在人体姿态估计中尤其有用，因为它可以帮助模型理解身体各部位之间的空间关系。
            part_feats = self.branch_2d_nonlocal(part_feats)
        return part_feats

    def _get_3d_smpl_feats(self, features, part_feats):
        if self.use_keypoint_features_for_smpl_regression:#这里被设置为false,
            smpl_feats = part_feats
        else:
            smpl_feats = self.smpl_deconv_layers(features)
            if self.use_branch_nonlocal:#default=null
    ...
    def _get_part_attention_map(self, part_feats, output):
        heatmaps = self.keypoint_final_layer(part_feats)#这里形成了最后的通道
        ...
        if self.use_heatmaps == 'hm':
            ...
            output['pred_kp2d'] = pred_kp2d
            output['pred_heatmaps_2d'] = heatmaps
        elif self.use_heatmaps == 'part_segm':#pare_config中配置的是part_segm
    ...
    def _pare_get_final_preds(self, pose_feats, cam_shape_feats, init_pose, init_shape, init_cam):
        ...
        if self.iterative_regression:#默认值为false，不知道为啥子
        ...
        else:#如果不使用迭代回归，则直接通过MLP预测姿态、相机参数和形状。
            shape_feats = cam_shape_feats
            if self.use_final_nonlocal:#默认为null
