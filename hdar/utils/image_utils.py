...
def generate_heatmaps_2d(joints, joints_vis, num_joints=24, heatmap_size=56, image_size=224, sigma=1.75):
    ...
    '''
    #热图是一种表示关键点位置的图像，其中每个关键点的位置被一个高斯分布所标记。以下是对这段代码的详细解释
    target_weight = np.ones((num_joints, 1), dtype=np.float32)#用于存储每个关节的权重（可见性），初始值为1。
    target_weight[:, 0] = joints_vis[:, 0]

    target = np.zeros((num_joints, heatmap_size, heatmap_size), dtype=np.float32)#用于存储每个关节的热图

    tmp_size = sigma * 3#计算高斯分布的临时尺寸，用于确定高斯分布的范围。

    # denormalize joint into heatmap coordinates
    joints = (joints + 1.) * (image_size / 2.)

    for joint_id in range(num_joints):
        feat_stride = image_size / heatmap_size#计算特征步长，将图像坐标映射到热图坐标的尺度
        mu_x = int(joints[joint_id][0] / feat_stride + 0.5)#获取关节的 x 坐标在热图中的位置
        mu_y = int(joints[joint_id][1] / feat_stride + 0.5)
        # Check that any part of the gaussian is in-bounds
        ul = [int(mu_x - tmp_size), int(mu_y - tmp_size)]# 高斯分布左上角的坐标
        br = [int(mu_x + tmp_size + 1), int(mu_y + tmp_size + 1)]# 高斯分布右下角的坐标
        # 检查高斯分布是否在热图范围内，如果不在，则跳过该关节
        ...
        if v > 0.5:#如果关节可见（权重大于0.5），则将高斯分布的值赋给热图上对应的区域。
