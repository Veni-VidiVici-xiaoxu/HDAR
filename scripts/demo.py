...
def main(args):
    ...
    if demo_mode == 'video':
        ...
        #wmh_change
        output_path_opt = os.path.join(args.output_folder, input_image_folder.rstrip('/').split('/')[-1] + '_' +'opt'+ args.exp)
        os.makedirs(output_path_opt, exist_ok=True)

        output_img_folder_opt = os.path.join(output_path_opt, 'pare_results')
        os.makedirs(output_img_folder_opt, exist_ok=True)
        #wmhend---------------------
    ...
    if args.mode == 'video':
        ...
        total_time = time.time()
        detections = tester.run_detector(input_image_folder)#测试了，这里是正常的
        print('--------------------------------------------------有么-------')
        print(detections)
        pare_time = time.time()
        ...
        #     vis_keypoints=True,
        #     device='cuda:0',
        #     mmpose_cfg='/home/aita/uaitadisknew/wmhcode/PARE-master/mmpose-main/configs/wholebody_2d_keypoint/topdown_heatmap/coco-wholebody/td-hm_hrnet-w48_8xb32-210e_coco-wholebody-256x192.py',
        #     mmpose_ckpt='/home/aita/uaitadisknew/wmhcode/PARE-master/mmpose-main/checkpoints/hrnet_w48_coco_wholebody_256x192-643e18cb_20200922.pth'
        # )
        #wmh_change

        tester.run_on_image_folder2(input_image_folder, detections, output_path_opt, output_img_folder_opt,
                                   run_smplify=args.smplify)
        #change_end
        end = time.time()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--mode', default='folder', choices=['video', 'folder', 'webcam'],
                        help='Demo type')#wmh_change，我把默认值改成了folder
